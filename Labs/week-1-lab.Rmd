...
## what class of variable are you working with?

Great, home ownership is categorized as factor, which means values correspond to one of a fixed number of categories. Interest rate is categorized as numeric. 
...
```{r}
head(loans_full_schema$homeownership) # we'll want to make that lowercase to make it tidy

# factor re-level to put them in a sensible order

loans <- loans_full_schema %>% 
  mutate(homeownership = tolower(homeownership), 
         homeownership = fct_relevel(homeownership, "rent", "mortgage", "own"), #the variable you want to relevel, then write the levels out
...
p_count <- ggplot(loans, aes(x = homeownership)) +
  geom_bar(fill = "darkgreen") +  #default setting is "count"
  labs(x="Homeownership", y = "Count") +
  theme_classic()
p_count

# Bar plot showing proportion of loans in each homeownership category
p_proportion <- loans %>%  #original data
  count(homeownership) %>% #count makes a new variable in your data set by default "n"
  mutate(proportion = n/sum(n)) %>%  #creating a proportion, so then we can plot it
  ggplot(aes(x=homeownership, y = proportion)) +
  geom_col(fill = "darkgreen") + #geom_col() creates columns of anything you want; geom_bar() doesn't allow you to specify y axis
...
loans %>% 
  ...
  pivot_wider(names_from = homeownership, values_from = n) %>%  #first, output column you want to call, second, what you want the values to be
...
# type of applicant differece systematically 
...
```
The histogram is right skewed. Unimodal (possibly missing data and it is bimodal).
...
```{r hist2}
ggplot(loans, aes(x = interest_rate)) +
  geom_histogram(breaks = seq(5, 27.5, 2.5)) + #start at 5, go to 27.5, in intervals of 2.5
  labs(x = "Interest Rate", y = "Count") + 
  theme_classic()

ggplot(loans, aes(x = interest_rate)) +
  geom_histogram(breaks = seq(5, 27.5, 2.5), aes(y = after_stat(density))) + #start at 5, go to 27.5, in intervals of 2.5
  labs(x = "Interest Rate", y = "Count") + 
  theme_classic()
...
## Box plots: A nice [standard] way to look at quantiles [usually quartiles]
...
p_boxplot = ggplot(loans, aes(x = interest_rate)) +
  geom_boxplot(lower = 25, middle = 50, upper = 75) +
  labs(x = "Interest Rate") +
  theme_classic() +
  theme(axis.text.y = element_blank(), #can add bold, can get rid of y axis
...
Very skewed. Lots of outliers (denoted by the dots). Median is slightly off center to the left, so a tail to the right.
